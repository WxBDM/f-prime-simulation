...
class Events(Logger):
    ...
    def __init__(self):
        super().__init__() # allows for all logger methods to be used here.
    
    def acquire_sun_position(self):
        event = self.create_new_event()
        event.add_string("Acquire sun position algorithm.")
        self.add_to_logger(event)

    def initiate_sun_pointing_algorithm(self):
        event = self.create_new_event()
        event.add_string("Initiating sun pointing algorithm.")
        self.add_to_logger(event)
        
    def charge_satellite(self):
        event = self.create_new_event()
        event.add_string("Charging satellite batteries.")
        self.add_to_logger(event)

    def terminate_sun_pointing_algorithm(self):
        event = self.create_new_event()
        event.add_string("Terminating sun pointing algorithm.")
        self.add_to_logger(event)
    
    def get_log(self):
        return self.events # inherits events dataframe, returns it.


events = Events()

events.acquire_sun_position()
events.initiate_sun_pointing_algorithm()
events.charge_satellite()
events.terminate_sun_pointing_algorithm()

print(events.get_log())
